...
const Gallery = () => {
  ...
  const events = [
      ...
      images: [
        {
          ...
          caption: "Guest lecture on 5g connectivity"
        },
        {
    ...
    },
    {
      ...
      location: "Own Location (Round 1) & IIEC (CB 207) (Round 2)",
      attendees: "Various",
  ...
  const nextImage = (eventId) => {
    setCurrentEventIndex(prev => ({
      ...prev,
      [eventId]: ((prev[eventId] || 0) + 1) % events.find(e => e.id === eventId).images.length
    }));
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {events.map((event, eventIndex) => (
              ...
              >
                {/* Event Header */}
                <div className="mb-8">
                  <h2 className="text-2xl md:text-3xl font-bold mb-4 apple-text-gradient cursor-pointer hover:opacity-80 transition-opacity" onClick={() => navigate(`/event/${event.id}`)}>
                    {event.title}
                  </h2>
                  <div className="flex flex-col gap-2 text-apple-gray-600 dark:text-apple-gray-300 mb-4">
